...
public class UserController {
   ...
   int attempt= 0;
    ...
    @Retry(name = "catalogServiceName", fallbackMethod = "fallBackFindByName")
    public ResponseEntity findByName(@PathVariable("name") String userName){
        ...
        if(userName.equals("array")){
            String[] arr = {"Rohit","Shikar","Virat","Dhoni"};
            for(int i=0;i<=arr.length;i++)
                System.out.println(arr[i]);
        }else if(userName.equals("index"))
            list.get(77);
        System.out.println(catalogFeignClient.findByName(userName));
        //System.out.println(restTemplate.getForObject("http://localhost:8083/catalog/api/all", ArrayList.class));
        //return ResponseEntity.status(HttpStatus.OK).body(catalogFeignClient.findByName(userName));
        return ResponseEntity.status(HttpStatus.OK).body(list.stream()
                .filter(e -> e.getName().equals(userName)).findAny().orElse(null));
    }
    public ResponseEntity fallBackFindByName(Exception ex) {
        System.out.println("fallBackFindByName method  "+ ex.getMessage());
        return ResponseEntity.status(HttpStatus.OK).body("try after some time "+ex.getMessage());
    }
